import { VrstaBiljke, PeriodPreporuke, MesecInterval } from '../../core/models/domain.models';

const m = (mesecOd: number, mesecDo: number): MesecInterval => ({ mesecOd, mesecDo });

/**
 * Preporučeni periodi sadnje i berbe po vrsti biljke. Moraju biti u skladu
 * sa `backend/src/biljka/periodi.util.ts` (isti izvor istine).
 */
...
/** Da li dati ISO datum (yyyy-MM-dd) upada u bar jedan od zadatih meseci-intervala. */
export function jeDatumUPeriodu(datumISO: string, intervali: readonly MesecInterval[]): boolean {
  if (!datumISO) return true;
  const mesec = new Date(datumISO).getMonth() + 1;
